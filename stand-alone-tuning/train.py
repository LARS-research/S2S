...
#        test_data = torch.LongTensor(get_data_idxs(d.test_data, entity_idxs, relation_idxs))[0:512]
    ...
    args.perf_file = os.path.join(directory, args.dataset + '_search_nCP_nary_' + str(args.num_blocks) + "_" + str(args.trial) + '.txt')
    ...
    mrr = model.train(train_data, valid_data, tester_val, tester_tst, tester_trip_class)
...
#    parser.add_argument('--n_arity', type=int, default=2, help='')
    ...
    parser.add_argument('--trial', type=str, default="3", help='')
    ...
    space4kge = {
        ...
        "n_batch": hp.choice("n_batch", [128]),
        #"n_dim": hp.choice("n_dim", [512]),
        #"lamb": hp.uniform("lamb", 0, 0.1)
    }
        
    x = np.zeros((4,4,4,4))
    x[0,0,0,0], x[1,1,1,1], x[2,2,2,2], x[3,3,3,3] = 1, 1, 1, 1
    arch = torch.LongTensor(x).view(-1).tolist()
    
#    x = [2, 1, 0, 1, 0, 1, 2, 0, 0, 0, 1, 0, 1, 1, 0, 0, 2, 2, 2, 0, 0, 0, 2, 2, 0, 2, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 0, 2, 2, 2, 1, 2, 0, 0, 1, 0, 0, 1, 0, 2, 2, 2, 1, 2, 2, 2, 1, 1, 1, 0, 1, 0, 1, 0, 1, 1, 1, 1, 2, 1, 2, 0, 2, 0, 2, 0, 0, 0, 0, 0, 2, 1, 2, 1, 2, 2, 2, 2, 0, 0, 2, 2, 1, 1, 0, 2, 2, 2, 2, 2, 1, 2, 2, 2, 1, 1, 0, 0, 1, 1, 1, 0, 1, 2, 2, 2, 1, 2, 2, 1, 1, 1, 1, 0, 1, 0, 0, 2, 1, 1, 1, 0, 2, 0, 2, 2, 2, 2, 2, 0, 1, 0, 2, 1, 2, 0, 0, 1, 2, 2, 2, 1, 1, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 0, 1, 1, 1, 2, 2, 0, 1, 2, 0, 0, 1, 1, 1, 1, 1, 0, 1, 2, 1, 1, 0, 2, 1, 1, 0, 2, 1, 1, 2, 0, 2, 2, 2, 2, 2, 0, 1, 1, 2, 0, 1, 1, 0, 1, 2, 0, 1, 0, 2, 2, 2, 0, 2, 1, 2, 1, 0, 2, 1, 0, 1, 1, 1, 1, 2, 1, 1, 2, 2, 2, 0, 2, 2, 0, 2, 0, 1, 1, 0, 1, 0, 0]    
#    arch = torch.LongTensor(x).view(-1).tolist()
    
    def f(params):
        mrr = main(params, args, arch)
